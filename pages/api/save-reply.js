...
async function saveReplyInDatabase(comment) {
	const { post_id, user_id,parent_comment_id, content, date } = comment

	const sql = 'INSERT INTO `postcomments`(`comment_id`, `post_id`, `user_id`, `parent_comment_id`, `content`, `date_created`, `is_edited`) VALUES (NULL, ?, ?, ?, ?, ? , 0)'
	const values = [post_id, user_id,parent_comment_id, content, date]
